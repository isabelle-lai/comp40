 ...
 *
 *    Summary: Implementation file for module of conversion of rgb int to 
 *             component video format and the corresponding decompression step 
 *             which converts it scaled rgb int pixel format
 *
 */
...
/* CONSTANTS */
const unsigned DENOMINATOR = 30000;
...
static void copy_pixel(int col, int row, A2Methods_UArray2 image,
                       void *elem, void *cl);

/* RGB INT AND FLOAT HELPER FUNCTIONS */
static void convert_rgb_to_float(Pnm_rgb int_pixel, 
                                 RGB_Float_Pixel float_pixel,
                                 unsigned denominator);
static void convert_float_to_rgb(Pnm_rgb int_pixel, 
                                 RGB_Float_Pixel float_pixel, 
                                 unsigned denominator); 
static float bound_float(float num);

/* RGB FLOAT AND COMPONENT VIDEO HELPER FUNCTIONS */
static void float_to_comp_vid_pixel(RGB_Float_Pixel float_pixel, 
                                    Comp_Vid_Pixel cv_pixel);
static void comp_vid_pixel_to_float(RGB_Float_Pixel float_pixel, 
                                    Comp_Vid_Pixel cv_pixel);

/* APPLY FUNCTIONS */
static void rgb_to_cv_apply(int col, int row, A2Methods_UArray2 cv_image, 
                            void *elem, void *cl);
static void cv_to_rgb_apply(int col, int row, A2Methods_UArray2 cv_image, 
                            void *elem, void *cl);
 ...
 *
 * Purpose: Reads in original pnmppm (in rgb pixels), trims it, and then 
 *          converts the rgb_pixels to component video pixels. 
 * 
 * Effects: Checked runtime error if image provided is null, malloc fails or 
 *          created new image is null. Frees memory associated with the 
 *          trimmed image. 
 */
Pnm_ppm cv_image_compress(Pnm_ppm original)
{
    ...
    /* 
     * creates new image with the same attributes as original image except for 
     * the pixels, which will be created using the map function and converted 
     * to comp_vid pixels using an apply function
     */
    ...
    new_image -> pixels = (A2Methods_UArray2) new_image -> methods -> 
                          new(new_image -> width, new_image -> height, 
                              get_comp_vid_size());
    new_image -> methods -> map_default(new_image -> pixels, rgb_to_cv_apply, 
                                        trimmed_image);
 ...
 *
 * Purpose: Reads in original pnmppm that is in component video form and 
*           converts it into rgb_pixel (scaled integers) format. 
 * 
...
Pnm_ppm cv_image_decompress(Pnm_ppm original)
{
    assert(original != NULL);
    /* 
     * creates new image with the same attributes as original image except 
     * for the pixels, which will be created using
    ...
    new_image -> methods -> map_default(original -> pixels, cv_to_rgb_apply, 
                                        new_image);    
    return new_image;
}
 ...
 *
 * Purpose: Trims an image to so that the width and height are even numbers 
 * 
 * Effects: Checked runtime error if image provided is null or malloc fails. 
 *          Copies pixels from original image to the trimmed image based on 
 *          new height and width using map and apply
 */
static Pnm_ppm trim(Pnm_ppm image)
{
    assert(image != NULL);
    ...
    /* 
     * Checks to make sure that the width and height are even numbers if it is,
     * it will decrease the width or height by 1
     */
    if (image -> width % 2 == 1) {
    ...
    trimmed_image -> pixels = (A2Methods_UArray2) trimmed_image -> methods -> 
                              new(trimmed_image -> width, 
                                  trimmed_image -> height, 
                                  sizeof(struct Pnm_rgb));
    trimmed_image -> methods -> map_default(trimmed_image -> pixels, 
                                            copy_pixel, image);
 ...
 * Output: N/A
 *
 * Purpose: Apply function for trimming to copy all the original pixels at the 
 *          original image to the new image
 * 
 * Effects: current pixel of new image is set to the value of the current pixel
 *          at the origunal image. 
 */
static void copy_pixel(int col, int row, A2Methods_UArray2 image, 
                       void *elem, void *cl) 
{
    ...
    Pnm_rgb curr_pixel = (Pnm_rgb) original_image -> methods -> 
                         at(original_image -> pixels, col, row);
 ...
 * 
 * Effects: Takes original image's current rgb pixel and converts it to float 
 *          rgb pixel and then to component video pixel and set it to the 
 *          current pixel of new image
 */
static void rgb_to_cv_apply(int col, int row, A2Methods_UArray2 cv_image, 
                            void *elem, void *cl)
{
    ...
    Pnm_rgb rgb_pixel = rgb_image -> methods -> 
                        at(rgb_image -> pixels, col, row);
 ...
 * Output: N/A
 *
 * Purpose: Apply function for transforming each component video in original 
 *          image to rgb pixel format and putting it in the new image
 * 
 * Effects: Takes original image's current cv pixel and convert it to float 
 *          rgb pixel and then to scaled int rgb pixel and set it to the 
 *          current pixel of new image
 */
static void cv_to_rgb_apply(int col, int row, A2Methods_UArray2 cv_image, 
                            void *elem, void *cl)
 ...
 */
static void convert_rgb_to_float(Pnm_rgb int_pixel, RGB_Float_Pixel float_pixel,
                                 unsigned denominator)
{    
 ...
 */
static void convert_float_to_rgb(Pnm_rgb int_pixel, RGB_Float_Pixel float_pixel,
                                 unsigned denominator) 
{
